# 0, 1, 4, 4, 5, 7
class Solution:
    def findPairsLessThanLower(self, nums: List[int], lower) -> int:
        l, r = 0, len(nums)-1
        count = 0

        while l < r:
            while l < r and r < len(nums) and nums[l] + nums[r] < lower:
                count += (r-l)
                l += 1
            r -= 1
        return count
    ...
    def countFairPairs(self, nums: List[int], lower: int, upper: int) -> int:
        nums.sort()
        pairs_less_than_lower = self.findPairsLessThanLower(nums, lower)
